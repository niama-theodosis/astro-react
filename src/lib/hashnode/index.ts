...
import {PageInfoFragment, PostCardFragment, PostExtrasFragment, PostTagsFragment} from "./fragments"
...
export async function fetchPosts(first: number, after?: string) {
  ...
  const title = publication?.displayTitle ?? publication?.title ?? "Blog"
  const tagline = publication?.about?.text ?? "Découvrez nos articles"
  return {items: (publication?.posts.edges ?? []).map(({node}) => node), tagline, title}
}
